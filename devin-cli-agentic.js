// NOTE: still needs fixing — requires a logged-in Devin account (`devin auth login`);
// real output is <summary>-wrapped and non-deterministic; catalog is a static subset of
// `devin models list`. Not production-ready.
export const DEVIN_MODEL_CATALOG = [
  ...
  // Claude Opus 4.6
  { id: "claude-opus-4-6-thinking-1m", name: "Claude Opus 4.6 Thinking 1M", contextLength: 1000000 },
...
export default {
  ...
  display: {
    ...
    notice: {
      ...
      text: "Claude-format ACP summarizer bridge. Installs the official Devin CLI then runs `devin auth login`; auth is owned by the Devin CLI. No host API key.",
    },
  },
  category: "free",
  authType: "none",
  ...
  models: DEVIN_MODEL_CATALOG.map((m) => ({
    id: m.id,
    name: m.name,
