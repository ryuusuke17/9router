# v0.5.50.2 (2026-08-12)

## Removal
- **Raycast**: removed Raycast Pro AI provider integration and all associated OAuth/configuration routes
- **Devin CLI**: removed Devin CLI Agentic and Devin CLI provider integrations, including executors, registry entries, and all configuration routes
- **Cleanup**: removed 20 tracked integration files and unwired 10 sites across the codebase; loose untracked root `.js` files left intentionally untouched

## Merge
- Merged upstream v0.5.50 (2026-08-05) into local fork, keeping local fixes: cookie authType routing, kimi-web endpoint fix, web-cookie provider validators, static model catalogs
- Version bumped to v0.5.50.2 for both `9router-app` and the `9router` CLI package.
...
- **Endpoint**: auto-provision a "Default Key" for first-time users so `/v1` works
...
- **DB**: `ENABLE_REQUEST_LOGS` env var now overrides the UI setting correctly; observability defaults to off (opt-in)
- **Translator**: preserve Codex Responses Lite tool use across chat-native OpenAI-compatible providers
- **Translator**: don't drop image-only user messages in `prepareClaudeRequest`
- **Translator**: drop JSON Schema keywords Gemini rejects (`uniqueItems`, `contains`, `multipleOf`, `unevaluatedProperties`, `unevaluatedItems`, `contentSchema`)
- **Claude**: remove global header cache that leaked one client's identity onto another client/account sharing the server; gate `anthropic-beta` by model instead
- **Antigravity**: drop retired Gemini 3.0 quota tiers, show Gemini 3.6 Flash usage bars
- **Cloudflare AI**: declare API key authentication (dashboard showed "No connections" despite an active key)
- **GitHub Copilot**: hold monthly-exhausted accounts until UTC month reset instead of only cooling down 120s
- **Providers**: count apikey connections for Ollama free-tier card; free-tier/apikey providers without `authModes` now default to apikey (were treated oauth-only)
- **Build**: include static/public assets in standalone output (login page hung on 404s when run via PM2)
- **Server**: support IntelliJ IDEA OpenAI-compatible clients over HTTP (h2c upgrade handling)
- **Auth**: redirect already-logged-in sessions away from `/login`
- **CLI tools**: enable Apply button for dynamic OpenAI/Anthropic-compatible provider connections
- **CLI**: include complete API artifacts in the CLI package
- **TTS**: a bare self-hosted model name is the MODEL, not the voice "kokoro" was parsed as a voice against a default model, 404ing or synthesising with the wrong one
- **Embeddings**: self-hosted embeddings no longer fall back to `api.openai.com` when a connection has no `baseUrl` " that silently sent the input text and API key to OpenAI under a provider named "Self-hosted"
- **Embeddings**: an adapter that rejects a misconfigured connection now returns 400 with the reason instead of escaping the handler uncaught
- **Embeddings**: bound the upstream fetch with `FETCH_CONNECT_TIMEOUT_MS` " an endpoint that drops packets never returns headers, so the request previously hung indefinitely
...
## Features
- **TTS**: add Xiaomi MiMo text-to-speech (preset voices �+��3-/�O%�Z%/�<?�%"/�T����/Mia/Chloe/Milo/Dean, style control, language hint dropdown with Auto-detect, i18n for Style label/placeholder)
...
- **Kimi**: normalize reasoning_effort to backend (#2427)
...
- **Kiro**: add Claude Opus 5 model support (#2264) " Edison42
- **Xiaomi-tokenplan**: region selector, key validation, multi-connection (#2251) " MiQieR
- **Translator**: strict Anthropic content block compliance (#2225) " whale9820
...
- **Codebuddy-cn**: show one-shot bonus packs as expiring, not monthly-replenishing " zmf
...
- Preserve reasoning effort through Codex translations " ndtung6868
...
- **CodeBuddy CN**: new OAuth provider (copilot.tencent.com) " 15-model catalog, /v2 inference, forced streaming, OpenIstyle reasoning
...
- Stop double-counting streaming usage at source " decolua
- Usage logging dedupe to reduce stats churn " Mink Nguyen
- Prevent non-SSE stream pipe crash and cross-IdP account overwrites (#2046) " qianze
- Resolve Gemini TTS models from catalog " nguyenha935
- Support Kiro IDC (organization) token import " quanturbo
- Preserve forced streaming for JSON clients (#2031) " Joseph Yaksich
- Preserve Responses text format (Codex) " tenglong
...
## Fixes
- DeepEcho thinking mode: echo `reasoning_content` back on follow-up turns to avoid 400 (#1543)
...
- Tunnel: skip virtual interfaces to prevent false "checking" when tunnel URL is reachable
...
- GitHub Copilot: refresh missing/expired token on models discovery (#1435) + add mappable gpt-5-mini/gpt-5.4-nano slots for Copilot MITM (#1426)
