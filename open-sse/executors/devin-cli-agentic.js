...
import { DEVIN_MODEL_CATALOG } from "../providers/registry/devin-cli-agentic.js";
import { buildErrorBody } from "../utils/error.js";
import {
  buildClaudeSseFrames,
  buildClaudeTextResponse,
  buildClaudeToolUseResponse,
} from "./devin-agentic/anthropicResponse.js";
import { serializeAnthropicForDevin } from "./devin-agentic/serializer.js";
import { parseDevinToolRequest } from "./devin-agentic/toolParser.js";
import { asRecord, DevinAgenticBridgeError, estimateTokens } from "./devin-agentic/types.js";
...
export function assertLocalAcpUrl(url) {
...
export function buildDevinChildEnv(_credentials, source = process.env) {
  const home = source.DEVIN_AGENTIC_HOME?.trim() || "";
  if (!home || !path.isAbsolute(home) || !isIsolatedHome(home)) {
    // No explicit isolated sandbox: fall back to inheriting the parent env
    // (like the plain devin-cli executor) so local installs work without a
    // container. Strip Claude/Anthropic secrets and force bypass permission mode.
    const env = { ...source };
    // The executor passes HOME as ACP session cwd, which the real Devin CLI
    // requires (omitting it → -32602 "Invalid params"). Windows does not set
    // HOME, so synthesize one from USERPROFILE/HOMEDRIVE+HOMEPATH/homedir().
    env.HOME = source.HOME || source.USERPROFILE || (path.join(os.homedir()));
    env.DEVIN_PERMISSION_MODE = source.DEVIN_PERMISSION_MODE || "bypass";
...
function errorBody(error) {
  ...
  const body = buildErrorBody(bridge?.status || 500, sanitizeErrorMessage(String(bridge?.message || (error instanceof Error ? error.message : String(error)))));
  body.error.type = bridge?.code || "devin_agentic_error";
  body.error.code = bridge?.code || "devin_agentic_error";
  return body;
}

export async function runAcpTurn(args) {
  const timeoutMs = Number(process.env.DEVIN_AGENTIC_ACP_TIMEOUT_MS || 120000);
  // The official CLI is a native binary; a mock is a JS module. When the resolved
  // bin is a mock .js/.mjs/.cjs, spawn it through the current runtime (node/bun).
  const isMockModule = /\.(m?js|cjs)$/i.test(args.devinBin);
  const cmd = isMockModule ? process.execPath : args.devinBin;
  const cmdArgs = isMockModule
    ? [args.devinBin, "acp", "--agent-type", "summarizer"]
    : ["acp", "--agent-type", "summarizer"];
  ...
  return await new Promise((resolve, reject) => {
    ...
    initializeRequestId = send("initialize", {
      ...
      clientInfo: { name: "9router-devin-cli-agentic", version: "1.0" },
      clientCapabilities: {},
    });
  });
}
...
async function generateAgenticOutput(args, promptText) {
  return await runAcpTurn({ ...args, promptText });
}
...
export default DevinCliAgenticExecutor;