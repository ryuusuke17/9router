...
const user = users[0] || {};
let deviceId = user.analyticsId;
if (!deviceId) {
  try {
    const posthogPath = join(RAYCAST_SUPPORT, "posthog.distinctId");
    deviceId = JSON.parse(readFileSync(posthogPath, "utf-8"))["posthog.distinctId"];
  } catch {
    deviceId = null;
  }
}
